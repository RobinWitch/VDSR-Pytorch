...
This paper is friendly to newcomer, but this code is some difficult ,maybe just to me ?

Because the original author use MATLAB to generate train data...while I think MATLAB is hardly used nowadays.

In the code file eval.py ,I think im_gt_y is original photo array ,im_b_y is processed after Bicubic interpolation, im_h_y is used VDSR to generate at the foundation of im_b_y



## ATTENTION

Cause I can't run generate_train.m in my matlab by limited of my computer's memory , so i just make some modification in generate_train.m to use .bmp file only regardless of .jpg file in directoty 291.

What we generate is still train.h5, but I want to save original train.h5 in date directory ,so I rename my generated train.py as train1.py .

I think it's enough for me to confirm model vdsr is effective ,cause when I use train.h5 that given by author to train model ,the result PSNR is almost same as Bicubic interpolation.

You can download my generate train1.h5 at [百度网盘](https://pan.baidu.com/s/11DDexpewT_EGxFVsHYwtIw?pwd=kkc4)

or download the completed code at [百度网盘](https://pan.baidu.com/s/11DNrSkHzS1TACA8qPC7ueA?pwd=dijk)
